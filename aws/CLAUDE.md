...
The root module here owns the **shared** AWS resources — the main VPC and the single shared PostgreSQL RDS instance. Product-specific compute (ECS services, ECR repos, per-app DBs) lives in a sibling subdirectory per product under `aws/<product>/` — currently `aws/commerce/` (the `commerce` workspace; api + utils consolidated, see ADR-006).
...
- `vpc_id`, `public_subnet_ids`, `private_subnet_ids` — the shared VPC + subnets, consumed by product workspaces for their ALB/ECS networking
...
>
> **Knock-on:** app workspaces (commerce today, future products) configure `cyrilgdn/postgresql` to connect to RDS at plan time. When the instance goes private, every such workspace needs an execution-mode change or a TFC agent inside the VPC. Plan that on the same PR.
...
- `var.rds_password` is the master password. It's set via the `shared-rds-master` TFC Variable Set attached to both `platform-shared` and `commerce`, so rotation is a single edit. Back the value up in a password manager — **TFC sensitive variables are write-only**.
...
When standing up infrastructure for a new product (e.g. `financial-tracker-api`). One workspace per **product**, not per service (ADR-006) — api/utils/etc. are `.tf` files within it:

1. Create `aws/<product>/` with its own `terraform.tf` pointing at a new TFC workspace.
...
3. Add `data "terraform_remote_state" "platform"` reading `platform-shared` outputs (RDS connection info + `vpc_id`/subnets).
...
5. Consume the shared modules via `git::` source pinned to a ref:
   - `aws/modules/ecr` — per-image repo
...
6. Give the directory its own `CLAUDE.md` describing the product's deployment shape, plus a product-level `README.md`.
